...
//ha que insertar en nuevo presupuesto asignarle un estado de no validado, y agregarle sus detalles
  ...
  //definiendo el presupuesto general 
  $this->db->insert("presupuestos", $array_presupuesto);
  $id_presupuesto = $this->db->insert_id();

  //registrando la data que sera modificable 

  $array_data_editable= array(
    "nombre" => $nombre,
     "idCalendarioAcademico" => date("Y"),
     //realemnte aca es facultad 
      "idEscuela" => $facultad,
       "Ciclo" => $ciclo,
        "Revision" => 0,
         "Fecha" => date("y-m-d"),
  ...
  $this->db->insert("presupuestohorasclase", $array_data_editable);
   $id_presupuesto_editable = $this->db->insert_id();
...
//insertar el detalle del presupuesto que sera modificable 

$identificar_horario = explode("/", $contenido_arreglo_f[$i]);
$obtener_id= $this->db->query("select * from horario where HorarioTexto='".$identificar_horario[1]."' AND DiasTexto ='".$identificar_horario[0]."' AND Duracion ='".$identificar_horario[2]."'  ")->row_array();

$obtener_factor_duracion = $this->db->query("select * from horarioduracion where DuracionMin=".$identificar_horario[2])->row_array();
$factor_duracion = $obtener_factor_duracion["FactorPago"]; 

//hay q obtener la catedra...

$info_catedra = $this->db->query("select * from catedras where nombre= '".$contenido_arreglo_d[$i]."' ")->row_array();



$docente =  $contenido_arreglo_j[$i];
$id_docente = explode("-", $docente);
$info_preparacion= $this->db->query("SELECT * FROM docentes doc
JOIN docentemaestrias dm ON dm.`idMaestria` = doc.`idMaestria`
WHERE doc.`id` =".$id_docente[0])->row_array(); 



$array_modificable= array(
 "idPresupuestoHorasClase" => $id_presupuesto_editable,
 "Item" => $i,
 "Ciclo" => $ciclo,
 "idAsignatura" => $info_catedra["id"],
 
 "idSeccion" => $contenido_arreglo_e[$i],
 
  "idHorario" => $obtener_id["idHorario"],

 "idDocente" => $id_docente[0],

 "PERF" => $contenido_arreglo_m[$i],

 "INS" => $contenido_arreglo_h[$i],

 "AplicaPago" => $contenido_arreglo_l[$i],
 ...
 "Observaciones" => $contenido_arreglo_o[$i],
 "factorpago" =>    $factor_duracion,
 "maestriapago" =>   $info_preparacion["Pago"]
...
$this->db->insert("presupuestohorasclased", $array_modificable);
...
foreach ($usuarios as $key) {
    ...
    $objPHPExcel->setActiveSheetIndex(2)->setCellValue('B'.$iteracion, $key['id'].'-'.$key['nombres'].' '.$key['Apellidos']);
...
$catedrass = $this->db->query('select * from catedras')->result_array();
$iteracion = 2; 
$iteracion_corre = 1; 
foreach ($catedrass as $key) {
  $objPHPExcel->setActiveSheetIndex(2)->setCellValue('I'.$iteracion, $iteracion_corre);
    $objPHPExcel->setActiveSheetIndex(2)->setCellValue('J'.$iteracion, $key['nombre']);
...
for($i=9; $i<136; $i++){
  ...
  $this->setValidationCell($objPHPExcel,"I{$i}",'Docentes!$D$2:$D$200');
   ...
   $this->setValidationCell($objPHPExcel,"K{$i}",'Docentes!$H$2:$H$200');
    $this->setValidationCell($objPHPExcel,"D{$i}",'Docentes!$J$2:$J$200');
